...
import {
  Clock, ChevronRight, ChevronDown, AlertTriangle,
...
const PATHWAY_OPTIONS: { value: Pathway; label: string; description: string }[] = [
  { value: 'standard', label: 'Standard (5-Year)', description: '5 years as lawful permanent resident' },
  { value: 'marriage', label: 'Marriage (3-Year)', description: '3 years as LPR married to U.S. citizen' },
  { value: 'military', label: 'Military Service', description: 'Expedited for qualifying service members' }
]
...
export default function TimelinePage() {
  ...
  const pathwayInfo = selectedPathway === 'standard'
  ...
  const residencyDisplay = 'totalYears' in pathwayInfo
    ? (pathwayInfo as { totalYears: string }).totalYears
    : pathwayInfo.totalTime
  const earlyFilingDisplay = 'earlyFiling' in pathwayInfo
    ...
    <main className="min-h-screen">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 pt-28 pb-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-white/60">Estimate your naturalization journey from filing to oath</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 glass-panel p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Select Your Pathway</h2>
          <div className="grid md:grid-cols-3 gap-3">
            {PATHWAY_OPTIONS.map(option => (
              ...
              >
                <div className={`font-semibold ${selectedPathway === option.value ? 'text-cyan-400' : 'text-white'}`}>
                  {option.label}
                </div>
        ...
        <div className="mb-8 glass-panel p-6">
          <div className="grid md:grid-cols-3 gap-6 text-center">
            <div>
              <div className="text-sm text-gray-500 mb-1">Total Estimated Time</div>
              <div className="text-2xl font-bold text-white">{estimated.min}–{estimated.max} months</div>
              <div className="text-xs text-gray-500">from filing to oath</div>
            </div>
            <div>
              <div className="text-sm text-gray-500 mb-1">Residency Requirement</div>
              <div className="text-2xl font-bold text-cyan-400">{residencyDisplay}</div>
              <div className="text-xs text-gray-500">before filing</div>
            </div>
            <div>
              <div className="text-sm text-gray-500 mb-1">Early Filing</div>
              <div className="text-2xl font-bold text-emerald-400">{earlyFilingDisplay}</div>
              <div className="text-xs text-gray-500">before requirement met</div>
            </div>
          </div>
        </div>
        ...
        {/* Visual Timeline */}
        <div className="mb-8">
          <h2 className="text-xl font-bold text-cyan-400 mb-6">Naturalization Process Timeline</h2>
              ...
              <div key={phase.id} className="relative">
                ...
                >
                  <div className="flex items-center gap-4">
                    {/* Step number */}
                    <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${STATUS_COLORS[phase.status]} flex items-center justify-center flex-shrink-0`}>
                ...
                {/* Expanded details */}
                {expandedPhase === phase.id && (
                  <div className={`mt-1 p-5 rounded-lg border ${STATUS_BG[phase.status]}`}>
        ...
        <div className="mb-8 glass-panel p-6">
          ...
          <ul className="space-y-3 text-sm text-gray-300">
            <li className="flex items-start gap-2">
              ...
              <span>Processing times vary significantly by USCIS field office. Check <a href="https://egov.uscis.gov/processing-times/" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">USCIS processing times</a> for your specific location.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-cyan-400 mt-0.5">•</span>
              <span>Military applicants may qualify for expedited processing under INA § 328–329.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-cyan-400 mt-0.5">•</span>
              <span>The 90-day early filing window is calculated from when you meet the continuous residence requirement.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-cyan-400 mt-0.5">•</span>
              <span>FBI name check delays are the most common cause of extended processing times.</span>
            </li>
          </ul>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          >
            <div>
              <div className="font-semibold text-white group-hover:text-cyan-400">Processing Times by Office</div>
              <div className="text-sm text-gray-500">Current wait times at each field office</div>
            </div>
          ...
          >
            <div>
              ...
              <div className="text-sm text-gray-500">Track what you need for your pathway</div>
            </div>
            <ChevronRight className="w-5 h-5 text-gray-500 group-hover:text-cyan-400" />
          </Link>
        </div>
        ...
        <div className="mt-8 p-4 bg-gray-900/50 border border-gray-800 rounded-lg">
          <p className="text-xs text-gray-500 text-center">
            <strong>Disclaimer:</strong> Timeline estimates are based on publicly available USCIS data and may not reflect your individual case.
            Processing times change frequently. For advice about your specific situation, consult a licensed immigration attorney.
          </p>
        </div>
      </div>
    </main>
